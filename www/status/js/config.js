...
function save() {
	...
	var miner = $("#devices tbody tr:not(.unlock)").map(function() {
		...
		mods = mods.split(",").map(function(mod) {return parseInt(mod);});

		return {ip: ip, cgminer: [{port: port, mod: mods}]};
	}).get();
	...
	$.ajax({
		...
		data: {
			cfg: JSON.stringify(
				{zone: [{layers: 1, plot_split: 10, shelves: miner.length, miner: miner}]},
				null, '\t')
		},
		success: function() {},
	});
