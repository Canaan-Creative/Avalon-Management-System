...
#
$pattern = '/Ver\[([-0-9A-Fa-f]+)\]\sDNA\[([0-9A-Fa-f]+)\]\sLW\[([0-9]+)\]\sHW\[([0-9]+)\]\sDH\[([.0-9]+)%\]\sGHS5m\[([-.0-9]+)\]\sDH5m\[([-.0-9]+)%\]\sTemp\[([0-9]+)\]\sFan\[([0-9]+)\]\sVol\[([.0-9]+)\]\sFreq\[([.0-9]+)\]\sPG\[([0-9]+)\]\sLed\[(0|1)\]/';
$ip   = $_GET['ip'];
...
<table>
<tr>
  <th>Indicator</th><th>Device</th><th>MM</th><th>DNA</th><th>LocalWorks</th><th>DH%</th><th>GHS5m</th><th>DH%5m</th><th>T(C)</th><th>Fan(RPM)</th><th>ASIC V(V)</th><th>ASIC F(GHS)</th>
</tr>";
...
foreach($stats as $stat_name=>$stat) {
	if($stat_name !== 'STATUS' && strpos($stat['ID'],"POOL") === False){
		...
		foreach($stat as $key=>$value)
			...
			if(strpos($key,'MM ID') !== False) $mods[] = substr($key,5);
	...
	if ($matches[11])
        ...
        #echo "<td><button onClick=\"adjust_volt('" . $ip . "'," . $port . "," . substr($stat['ID'], 3) . "," . $mod . ", 7875);\">7875</button>";
        #echo "<button onClick=\"adjust_volt('" . $ip . "'," . $port . "," . substr($stat['ID'], 3) . "," . $mod . ", 8000);\">8000</button>";
        #echo "<button onClick=\"adjust_volt('" . $ip . "'," . $port . "," . substr($stat['ID'], 3) . "," . $mod . ", 8125);\">8125</button></td>";
        echo $td . substr($stat['ID'],0,3) . "-" . substr($stat['ID'], 3) . "-" . $mod . "</td>";
        #echo $td . $mod . "</td>";
        #echo $td . $stat['ID' . $mod . ' MM Version'] . "</td>";
	echo $td . $matches[1] . "</td>";
	echo $td . substr($matches[2], 12) . "</td>";
	echo $td . number_format($matches[3], 0, ".", ",") . "</td>";
